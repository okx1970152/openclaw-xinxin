 ...
 * - "embedding": 原版 Embedding 语义检索
 * 
 * 超级大龙虾项目可选择 "keyword" 使用关键词检索
 */
const DEFAULT_ENGINE: MemoryEngine = "embedding";
...
export function resolveMemoryBackendConfig(params: {
  ...
  const engine = params.cfg.memory?.engine ?? DEFAULT_ENGINE;
  
  if (backend !== "qmd") {
    return { backend: "builtin", citations, engine };
  }
